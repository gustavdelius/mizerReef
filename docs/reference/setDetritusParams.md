...
`detritus_params` slot).

## Details

All detritus-related parameters (capacity, decomposition rates, external
input) are stored in the `detritus_params` slot of the params object.
Resource interaction strengths are set in the species_params data frame.
This function supports flexible multi-resource interaction via the
