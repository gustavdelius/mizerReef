...
with `satiation = FALSE` — by default the carnivores — have no upper
limit on their intake, formally $`h_i(w) = \infty`$. For them the
feeding level collapses to
``` math
f_i(w) = 0,
```
so they consume everything they encounter and the absorbed rate is
simply $`\alpha_i E_i(w)`$. Resource consumers (herbivores and
detritivores) keep the type II response by default. This behaviour is
implemented by
...
```
floored at zero for $`w < 1`$ g. Here $`w_{max.i}`$ is the maximum
weight of group $`i`$, $`k_{sen}`$ (`sen_prop`) sets the overall rate,
and $`p_{sen}`$ (`sen_curve`) controls the steepness with which
...
numerical instability of an explicit Euler step.
...
```
where $`P_A`$ is the production rate (set to balance consumption at
steady state,
[`getAlgaeProduction()`](https://cmbeese.github.io/mizerReef/reference/getAlgaeProduction.md))
and $`c_A`$ is the **mass-specific** consumption rate
...
```
so that the total grazing rate is $`c_A B_A`$. (Herbivores are
non-satiating by default, so $`f_i(w) = 0`$ and the bracket equals
$`1`$.) Over a step the equation integrates to
``` math
